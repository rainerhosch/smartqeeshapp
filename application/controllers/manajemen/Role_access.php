...
class Role_access extends CI_Controller
{
    ...
    public function get_menu_access()
    {
        if ($this->input->is_ajax_request()) {
            ...
            $where = [
                'ur.role_id' => $data_post
            ];
            $dataRole = $this->user->get_role_user(['role_id' => $data_post])->row_array();
    ...
    public function delete_role_user()
    {
        if ($this->input->is_ajax_request()) {
            $input_post = $this->input->post();
            $delete = $this->user->delete_data('user_role', ['role_id' => $input_post['id']]);
            if ($delete) {
                $data = [
                    'code' => 200,
                    'status' => true,
